...
import {
  Title, Button, Group, Paper, Table, TextInput, Text, Badge, Modal, Stack, Select, ActionIcon, Divider,
} from '@mantine/core';
...
const FAKE_ORDERS: Order[] = Array.from({ length: 15 }, (_, i) => ({
  id: `ord-${i + 1}`,
  orderId: `#${1000 + i}`,
  customerName: ['John Doe', 'Jane Smith', 'Bob Wilson', 'Alice Brown', 'Carlos Garcia'][i % 5]!,
  productName: 'Basic T-Shirt',
  designThumbnail: '',
  status: (['pending', 'processing', 'completed', 'completed', 'cancelled'] as const)[i % 5]!,
  total: 15 + Math.round(Math.random() * 50 * 100) / 100,
  createdAt: new Date(Date.now() - i * 86400000 * 2).toISOString(),
}));

export function Orders() {
  const [orders] = useState<Order[]>(FAKE_ORDERS);
        ...
        {selectedOrder && (
          <Stack>
            <Group justify="space-between">
              ...
              <Text size="sm">{selectedOrder.orderId}</Text>
            </Group>
            <Divider />
            <Group justify="space-between">
              <Text size="sm" fw={500}>Customer</Text>
              <Text size="sm">{selectedOrder.customerName}</Text>
            </Group>
            <Divider />
            <Group justify="space-between">
            ...
            <Group justify="space-between">
              ...
              <Badge color={STATUS_COLORS[selectedOrder.status]} variant="light">{selectedOrder.status}</Badge>
            </Group>
            <Divider />
            <Group justify="space-between">
              <Text size="sm" fw={500}>Total</Text>
              <Text size="sm" fw={700}>${selectedOrder.total.toFixed(2)}</Text>
            </Group>
            <Divider />
            <Group justify="space-between">
              <Text size="sm" fw={500}>Date</Text>
              <Text size="sm">{new Date(selectedOrder.createdAt).toLocaleString()}</Text>
            </Group>
            <Divider />
            ...
            <Group justify="flex-end">
              <Button variant="default" onClick={() => setSelectedOrder(null)}>Close</Button>
            </Group>
          </Stack>
        )}
      </Modal>
            ...
            <Table.Tbody>
              {filtered.map((o) => (
                <Table.Tr key={o.id}>
                  ...
                  <Table.Td><Text size="sm" fw={500}>${o.total.toFixed(2)}</Text></Table.Td>
