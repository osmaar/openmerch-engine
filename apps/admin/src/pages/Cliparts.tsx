...
export function Cliparts() {
  ...
  const handleDelete = (id: string) => {
    confirm('Delete Clipart', 'Are you sure you want to delete this clipart? This action cannot be undone.', async () => {
  ...
  const handleBulkDelete = () => {
    ...
    confirm('Delete Cliparts', `Are you sure you want to delete ${selected.size} clipart(s)? This action cannot be undone.`, async () => {
  ...
  return (
    <div>
      <Group justify="space-between" mb="lg">
        ...
        <Group gap="xs">
          <Button variant="light" leftSection={<Plus size={16} />} onClick={() => setShowBulkUpload(true)}>Add Multiple</Button>
          <Button leftSection={<Plus size={16} />} onClick={() => setShowCreate(true)}>Add New Clipart</Button>
        </Group>
      </Group>

      {/* Single create modal */}
      <Modal opened={showCreate} onClose={() => { setShowCreate(false); setNewName(''); setNewCategories([]); setNewTags([]); setNewPrice(0); setNewFeatured(false); setNewActive(true); setNewPreview(null); setNewFileName(''); setNewFile(null); }} title="Add New Clipart" centered size="md">
        <Stack>
            ...
            {newPreview ? (
              <Stack gap="xs">
                ...
                <Group gap="xs">
                  ...
                  <Button variant="subtle" size="xs" color="red" onClick={() => { setNewPreview(null); setNewFileName(''); setNewFile(null); }}>Remove</Button>
                </Group>
              </Stack>
            ) : (
      ...
      {/* Bulk upload modal */}
      <Modal opened={showBulkUpload} onClose={() => { setShowBulkUpload(false); setBulkFiles([]); setBulkCategories([]); setBulkTags([]); setBulkPrice(0); }} title="Add Multiple Cliparts" centered size="lg">
        <Stack>
          ...
          <Group justify="flex-end" mt="sm">
            ...
            <Button onClick={handleBulkUpload} disabled={bulkFiles.length === 0}>Upload {bulkFiles.length} Clipart(s)</Button>
          </Group>
        </Stack>
      </Modal>
                    ...
                    <Table.Td>
                      <Group gap="sm">
                        <div style={{ width: 36, height: 36, borderRadius: 6, background: '#f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                          {c.fileUrl ? <img src={c.fileUrl.startsWith('/') ? `${API_BASE}${c.fileUrl}` : c.fileUrl} alt={c.name} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} /> : <Image size={16} color="#ccc" />}
                        </div>
                        <div>
                          <Text size="sm" fw={500}>{c.name}</Text>
                          {c.tags.length > 0 && <Group gap={4} mt={2}>{c.tags.slice(0, 2).map((t: string) => <Badge key={t} size="xs" variant="outline" color="gray">{t}</Badge>)}</Group>}
                        </div>
                      </Group>
                    </Table.Td>
                    <Table.Td><Group gap={4}>{c.categories.map((cat: string) => <Badge key={cat} size="xs" variant="light">{cat}</Badge>)}</Group></Table.Td>
                    <Table.Td><Text size="sm">{c.price > 0 ? `$${(c.price / 100).toFixed(2)}` : 'Free'}</Text></Table.Td>
                    <Table.Td>
